...
Новые пользователи создаются администратором в блоке «Пользователи дашборда». Для менеджера укажите его `managerId` из списка менеджеров. В Airtable создаётся специальная запись с `Компания = __DG_USER_CONFIG__`; пароль хранится в поле `Auth Password Hash`, а статус — в `Auth Status`.
